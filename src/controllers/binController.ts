...
export const createBin = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    if (!wasteType || !locationUrl || !userId || !locationName) {
      res.status(400).json({
        message:
          'Missing required fields: wasteType, locationUrl, locationName, or userId'
      })
      return
    }
...
export const updateBin = async (req: Request, res: Response): Promise<void> => {
  try {
    const { id } = req.params
    const { wasteType, locationUrl, locationName, status, fillLevel, weight } =
      req.body as {
        wasteType?: string
        locationUrl?: string
        locationName?: string
    ...
    const updatedBin = await Bin.findByIdAndUpdate(id, updateData, {
      new: true
    })
    ...
    logger.error('Bin update failed', error)
    res.status(500).json({ message: (error as Error).message })
  }
}
...
export const collectBin = async (
  ...
  try {
    ...
    logger.error('Collect Bin failed', error)
    res.status(500).json({ message: (error as Error).message })
  }
}
...
export const cancelBin = async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    ...
    logger.error('Cancel Bin failed', error)
    res.status(500).json({ message: (error as Error).message })
  }
}
